import {
	...
	campusTalksData,
	featuredData,
} from "../functions/preFetch";

function displayCollectives() {
	let data = collectivesData();
	return data.map((item) => {
		return (
			<div>
				<div className='flex items-center gap-2'>
					<div className='img w-auto  h-7'><img className="w-full h-full object-cover" src={item.img} alt="" /></div>
					<div className='h-auto flex flex-col'>
						<p className=''>{item.org}</p>
						<p className=' font-light text-gray-500 text-xs inline'>
							{item.members}
						</p>
					</div>
...
function displayCampusTalk() {
	let data = campusTalksData();

	return (
		<div className='blog p-4'>
			<p className=' font-extrabold text-gray-800'>Campus Talks</p>
			<ul>
				{data.map((item) => {
					return (
						<li className=' list-disc list-inside font-light text-sm'>
...
function displayFeatured() {
	...
	return (
		<div className='blog p-4'>
			<ul>
				<li className=' font-extrabold text-gray-800'>Featured</li>
				{data.map((item) => {
					return (
						<li className=''>
							<div className='flex'>
								<svg
									className='w-[18px] h-[20px] my-auto'
									viewBox='0 -0.5 25 25'
									fill='none'
								...
								<a href={item.url} className='font-light text-sm'>
									{item.title}
								</a>
							</div>
						</li>
...
export default function Featured() {
	return (
		<div className='flex relative flex-col gap-5'>
			<div className='News w-68.75 h-fit bg-amber-50 rounded-md right-1 border border-amber-300'>
				{displayCampusTalk()}
